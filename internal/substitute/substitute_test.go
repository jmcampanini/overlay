...
func TestApplySubstitution(t *testing.T) {
	tests := []struct {
		...
		{"nested ref in fallback passes through whole", "${PRE_ENV:-${PRE_EMPTY}}", "${PRE_ENV:-${PRE_EMPTY}}", nil, nil},
		{"nested missing ref not flagged", "${PRE_GONE:-${PRE_GONE2}}", "${PRE_GONE:-${PRE_GONE2}}", nil, nil},
		{"unprefixed outer keeps prefixed inner", "${HOME:-${PRE_ENV}}", "${HOME:-${PRE_ENV}}", nil, nil},
		{"escaped nested expression", "$${PRE_ENV:-${PRE_EMPTY}}", "$${PRE_ENV:-${PRE_EMPTY}}", nil, nil},
		{"unbalanced outer keeps inner literal", "${BROKEN${PRE_ENV}", "${BROKEN${PRE_ENV}", nil, nil},
		{"unbalanced outer with missing inner not flagged", "${BROKEN${PRE_GONE}", "${BROKEN${PRE_GONE}", nil, nil},
		{"balanced multi-nested passes through whole", "${OUTER:-${PRE_ENV}${PRE_EMPTY}}", "${OUTER:-${PRE_ENV}${PRE_EMPTY}}", nil, nil},
		{"balanced early brace keeps inner literal", "${{}${PRE_ENV}}", "${{}${PRE_ENV}}", nil, nil},
		{"passthrough then real ref", "${a.b}${PRE_ENV}", "${a.b}fromenv", []string{"PRE_ENV"}, nil},
		{"unbalanced run to EOF keeps trailing ref literal", "${BROKEN${PRE_ENV}${PRE_ENV}", "${BROKEN${PRE_ENV}${PRE_ENV}", nil, nil},
		{"balanced nested passthrough then real ref", "${X:-${PRE_ENV}}${PRE_ENV}", "${X:-${PRE_ENV}}fromenv", []string{"PRE_ENV"}, nil},
